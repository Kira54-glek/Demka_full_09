...
<br/>
Порядок выполнения 1 модуль: 11,1,3,Предна,2(Nat),apt update,4,5,6,7,8,9,10.
<br/>
...
></br>
>
> На всех устройствах, кроме **ISP**(нужно 2 первых, если не идет инет, то на ISP добавить и chattr +i /etc/resolv.conf   # чтобы не слетал!):
>```
...
(Порядок 2 модуль: (1 SAMBA? не факт),2,3,4,5,8,10.) (6,7,9 если остальное успел)
ДОРАБОТАТЬ
### Задание 1
